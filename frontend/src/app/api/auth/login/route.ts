...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ detail: "Invalid login payload" }, { status: 422 });
  }
  ...
  if (!token) {
    return NextResponse.json({ detail: "Login succeeded but auth cookie was missing" }, { status: 502 });
  }
