"""
app/routers/proposal_apply_planning.py
...
approval readiness) and produces a guarded execution plan per proposal.

Read-only. No state mutations, no apply execution, no config writes.
"""
...
router = APIRouter(
    prefix="/api/proposal-apply-planning",
    tags=["proposal-apply-planning"],
)
...
def get_pipeline_apply_planning(
    ...
    """
    Deterministic apply planning evaluation per pipeline.

    Chains the full intelligence pipeline and produces a guarded execution
    plan per proposal. Returns planning objects. Read-only. Nothing is applied
    or stored.
    """
    ...
    total_planned = sum(i["planned_count"] for i in items)
    total_combined = sum(i["combined_count"] for i in items)
...
def get_vertical_apply_planning(
    tenant_id: Optional[str] = Query(None, description="Scope to a single tenant"),
    window_days: int = Query(7, ge=1, le=90, description="Trend comparison window in days"),
    lookback_days: int = Query(30, ge=1, le=365, description="Health aggregation window in days"),
    limit: Optional[int] = Query(None, ge=1, le=100, description="Limit number of results"),
    ...
    """
    Deterministic apply planning evaluation per vertical.

    Same logic as /api/proposal-apply-planning/pipelines, grouped by vertical_id.
    """
