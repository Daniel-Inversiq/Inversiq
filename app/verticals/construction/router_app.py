...
from app.verticals.construction.email_render import render_estimate_ready_email
...
from app.verticals.construction.render_estimate import render_estimate_pdf_html
from app.verticals.construction.review_labels import review_label_nl
...
def _estimate_editor_initial_values(lead: Lead, overrides: dict) -> dict:
    ...
    return {
        ...
        "company_name": (company.get("company_name") or company.get("name") or estimate.get("branding_company_name") or "Inversiq").strip(),
...
def _apply_full_estimate_edit(
    *,
    ...
    company_name = str(editor_input.get("company_name") or "").strip() or "Inversiq"
...
def workflow_vertical_for_ui(stored_vertical: str | None) -> str:
    """Map registry/DB vertical ids to UI workflow keys."""
    v = (stored_vertical or "").strip().lower()
    if v in {"paintly", "painters_us", "painting"}:
        return "construction"
    return v or "construction"
...
def _compatible_vertical_ids_for_workflow(workflow: str) -> set[str]:
    mapping = {
        "construction": {"construction", "paintly", "painting", "painters_nl", "painters_us"},
    }
...
def app_dashboard(
    ...
    def _lead_status_ui(raw_status: str) -> tuple[str, str]:
        ...
        if st == "SENT":
            return "Verzonden", "bg-emerald-50 text-emerald-700"
        if st == "VIEWED":
            return "Bekeken", "bg-slate-100 text-slate-700"
        if st == "SUCCEEDED":
            return "Klaar om te versturen", "bg-emerald-50 text-emerald-700"
        if st == "NEEDS_REVIEW":
            return "Controle nodig", "bg-blue-50 text-blue-700"
        if st == "CONFIG_NEEDED":
            return "Prijsinstellingen ontbreken", "bg-amber-50 text-amber-700"
        if st in {"FAILED", "ERROR"}:
            return "Mislukt", "bg-rose-50 text-rose-700"
        if st == "RUNNING":
            return "Wordt verwerkt", "bg-slate-100 text-slate-700"
        if st == "NEW":
...
def app_billing_topup(
    ...
    price_id = (os.getenv(TOPUP_STRIPE_PRICE_ENV_KEY) or "").strip() or None
    if not price_id:
        raise HTTPException(status_code=400, detail="Top-up price not configured")
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail="Stripe error during top-up checkout session creation",
        ) from exc
    except RuntimeError as exc:
        raise HTTPException(
            status_code=500,
...
def app_leads(
    ...
    enabled = (tenant_obj.enabled_verticals if tenant_obj and tenant_obj.enabled_verticals else None) or ["construction"]
...
def app_lead_detail(
    ...
    # Parse estimate output fields for generic display (vertical-agnostic).
    _estimate_dict: dict = {}
    if lead.estimate_json:
        try:
            _estimate_dict = json.loads(lead.estimate_json)
        except Exception:
            pass
    ...
    context = _dashboard_context(
        ...
        db,
        {
            ...
            "vertical": getattr(lead, "vertical", None) or "construction",
            "estimate_summary": _estimate_dict.get("summary"),
            "estimate_rendered_text": _estimate_dict.get("rendered_text"),
            "quote_ui": quote_ui,
            "show_prices": show_prices,
            "pricing_ready": pricing_ready,
            "is_provisional": is_provisional,
...
def app_lead_approve(
    lead_id: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_user_html),
):
    """Mark a non-painting NEEDS_REVIEW lead as QUOTE_READY.

    Restricted to non-painting verticals — painting leads go through the
    existing review/send flow and must not be affected.
    """
    lead = (
        db.query(Lead)
        .filter(Lead.id == lead_id, Lead.tenant_id == str(current_user.tenant_id))
    ...
    if (getattr(lead, "vertical", None) or "construction") in {"construction", "paintly", "painting"}:
        raise HTTPException(status_code=400, detail="Approve not available for painting leads")

    if (getattr(lead, "status", "") or "").upper() == "NEEDS_REVIEW":
        lead.status = "QUOTE_READY"
        db.add(lead)
        db.commit()

    return RedirectResponse(url=f"/app/leads/{lead_id}?approved=1", status_code=303)
...
def send_estimate(
    ...
    if not ent.allowed:
        ...
        if ent.reason == "monthly_offer_limit_reached":
            ...
            if is_htmx:
                logger.info(
                    "SEND_ESTIMATE_EARLY_RETURN lead_id=%s reason=monthly_offer_limit_reached response=hx_banner",
                    lead_id,
                )
                usage_current_htmx = int(ent.usage_current or 0)
                upgrade_url_htmx = ent.upgrade_url or f"/app/billing?upgrade=1&feature={Feature.BASIC_SENDING.value}"
                banner_html = (
                    '<div id="limit-banner" class="border border-amber-200 bg-amber-50 px-4 py-4 mb-4">'
                    '<div class="flex items-start gap-3">'
                    '<div class="flex h-7 w-7 shrink-0 items-center justify-center rounded bg-amber-100 text-amber-600">'
                    '<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">'
                    '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>'
                    "</svg>"
                    "</div>"
                    '<div class="flex-1">'
                    '<div class="text-[12px] font-semibold text-amber-900">Maandlimiet bereikt</div>'
                    f'<div class="mt-0.5 text-[11px] text-amber-800">Je hebt <strong>{usage_current_htmx} van {limit}</strong> offertes gebruikt deze maand.</div>'
                    '<div class="mt-3 flex flex-wrap gap-2">'
                    '<a href="/app/billing?action=topup" class="inline-flex items-center gap-1.5 rounded bg-[#111318] px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-[#2a2d35]">'
                    '<svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">'
                    '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>'
                    "</svg>"
                    "Extra credits kopen"
                    "</a>"
                    f'<a href="{upgrade_url_htmx}" class="inline-flex items-center gap-1.5 rounded border border-[#d0d4dd] bg-white px-3 py-1.5 text-[11px] font-semibold text-[#111318] hover:bg-[#f5f6f8]">'
                    "Plan upgraden"
                    "</a>"
                    "</div>"
                    "</div>"
                ...
                return HTMLResponse(
                    banner_html,
                    headers={"HX-Retarget": "#quote-actions", "HX-Reswap": "innerHTML"},
                )
            ...
            usage_current = int(ent.usage_current or 0)
            upgrade_url_limit = ent.upgrade_url or f"/app/billing?upgrade=1&feature={Feature.BASIC_SENDING.value}"
            return RedirectResponse(
                url=(
                    f"/app/leads/{lead_id}"
                    f"?send_error=offer_limit"
                    f"&usage_current={usage_current}"
                    f"&usage_limit={limit}"
                    f"&upgrade_url={upgrade_url_limit}"
                ),
                status_code=303,
            )
    ...
    if is_htmx:
        from app.verticals.construction.router_htmx import render_quote_oob_response
